...
class ChatBridgeModMenu : ModMenuApi {
    override fun getModConfigScreenFactory(): ConfigScreenFactory<*> {
        return ConfigScreenFactory { parent -> if (ChatBridge.hasYacl()) buildYaclScreen(parent) else null }
    }

    private fun buildYaclScreen(parent: Screen?): Screen? {
        return try {
            val clazz = Class.forName("com.nayoguildbridge.config.ChatBridgeConfigManager")
